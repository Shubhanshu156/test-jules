...
public class ChatGPTAIProvider extends BaseAIProvider {
    private static final String PROVIDER_NAME = "ChatGPT";
    private static final String ENDPOINT = "https://api.openai.com/v1/chat/completions";

    public ChatGPTAIProvider(AIProviderConfig config) { super(config); }
    ...
    public AIReviewResponse analyzeCode(AIReviewRequest request) throws AIProviderException {
        try {
            ...
            if (response.statusCode() >= 400) throw new AIProviderException("OpenAI API error: " + response.body(), AIProviderException.ErrorType.SERVICE_UNAVAILABLE);
            return parseResponse(response.body(), request);
        } catch (Exception e) {
            throw new AIProviderException("OpenAI API call failed: " + e.getMessage(), e, AIProviderException.ErrorType.NETWORK_ERROR);
        }
    }
